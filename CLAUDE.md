...
- **Rôles d'administration (socle, PAS ENCORE ACTIF)** : `profiles.role` ∈ `user` | `admin` | `owner`, défini par la migration `supabase/migrations/0013_admin_roles.sql` — **cette migration n'a été appliquée à AUCUNE base** (ni locale ni prod) ; tant que ce n'est pas fait, `role` n'existe pas en base et rien de ce qui suit n'est en vigueur. Ce qui EST écrit et couvert par des tests unitaires (qui moquent Supabase — voir plus loin) sur cette branche : le modèle de rôles lui-même, les gardes de route `requiresAdmin` / `requiresOwner` (avec écran « Accès réservé »), et `src/services/adminService.ts` (écritures règles/errata/rôles). Ce qui N'EXISTE PAS encore : aucune UI d'administration réelle (au-delà des vues placeholder), et aucun `owner` n'a jamais été posé — la toute première attribution se fait **à la main en SQL**, il n'y a pas d'autre chemin par design. `role` n'est écrivable **ni en `update` ni en `insert`** via l'API PostgREST : la migration révoque `insert, update` sur `public.profiles` **au niveau TABLE** puis ré-accorde explicitement `insert`/`update` colonne par colonne sur `user_id, username` seulement (un `revoke` au niveau colonne seul ne suffit pas — Postgres consulte l'ACL de relation en premier, et Supabase accorde `all privileges` au bootstrap ; PostgREST ne filtre rien côté HTTP, il transmet la requête telle quelle et c'est PostgreSQL qui répond `permission denied for column` sur la colonne non accordée). Seule la RPC `set_user_role()` attribue `role`, et `owner` n'y est jamais attribuable. Corrections de règles dans `rules_overrides` (colonnes `kind`/`sort_order` avec défaut, `chapter` obligatoire — une règle ajoutée n'a pas de ligne source en face) + vue de fusion `rules_effective` → re-scraper reste sans risque. Journal `admin_audit` **append-only, écrit par des triggers** (un journal écrit par le client peut ne pas l'être). **Avant toute mise en service** : (1) appliquer `0013` (SQL Editor Supabase), (2) poser le premier `owner` à la main (`update public.profiles set role = 'owner' where user_id = '<uuid>';`), (3) lancer `node scripts/checkAdminRls.mjs` contre la vraie base — **ce script n'a encore jamais été exécuté**, les tests unitaires moquent Supabase et ne prouvent rien sur la RLS.

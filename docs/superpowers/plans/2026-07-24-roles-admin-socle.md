...
- **REPLI OBLIGATOIRE sur la table `rules`.** La vue `rules_effective` n'existe qu'une fois
  la migration `0013` appliquée — or cette application est **différée**. Sans repli, merger
  ce lot avant la migration ferait basculer `/regles/officielles` sur « Règles
  indisponibles » alors que 445 règles sont en ligne aujourd'hui : une régression de
  production, exactement le piège rencontré en Phase 1 avec `errata.json`.

  Dans `load()`, si la requête sur `rules_effective` échoue, réessayer sur `rules` avant
  d'abandonner :

...
| --- | ------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- |
| A1  | **Choix de zone Allié tours ≥ 2 (303.1)**        | Hors 1er tour, un Allié va TOUJOURS au Monde ; le contrôleur devrait pouvoir choisir Monde **ou** Havre-Sac à tout tour.                      | Étendre `playDestination` + `playFromHand({destination})` + prompt UI « Monde / Havre-Sac » à la pose d'un Allié (défaut Monde). Le moteur+bot déjà prêts (mouvement fait). | Moyen (UI) |
| A2  | **Mal d'invocation à la SORTIE (1821/414)**      | Un Allié qui sort du Havre-Sac ce tour peut-il attaquer immédiatement ? `arrivedTurn` n'est pas reposé au mouvement → à vérifier vs la règle. | Vérifier le rulebook ; si un Allié « fraîchement sorti » ne peut pas attaquer, reposer un jeton au mouvement `havreSac→monde`.                                              | Faible     |
| A3  | **414.2 pour le Héros**                          | `whyCannotMoveHero` ne vérifie pas « dressé » ; un Héros incliné peut se déplacer (incohérent avec les Alliés, `whyCannotMoveCreature`).      | Aligner `moveHero` sur `whyCannotMoveCreature` (ou déléguer). Risque : tests hero-movement.                                                                                 | Faible     |
| A4  | **Ciblage de JOUEUR (PA/PM adverse)** ~29 effets | Le moteur ne cible que des instances ; « le joueur de votre choix perd 1 PA » reste manuel.                                                   | En 1v1, cible-joueur = l'adversaire → op `playerLoseStatTurn`/`playerGainStat` déjà là mais ciblage-joueur générique à câbler.                                              | Faible     |

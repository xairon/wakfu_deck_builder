...
**Dernière mise à jour : 2026-07-02.** État : **447 / 1802 effets imprimés structurés (24,8 %)** — `auto 401 · manual 46 · uncovered 1355 · ruling 452 · keyword 256 · trait 60`. Mesure : `npm run report-coverage`.
...
Cible en cours : `incarnam-feca / -cra / -iop / -xelor` (ids dans `src/data/officialDecks.ts`). Croisement decks × data : script jetable `/tmp/starter.mjs` (à recréer — voir §6). **48 → 35** (W37 noEquipment+tofu-mutant, W38 draw fromCount, W39 costDiscard, W40 oncePerTurn+tapsSource, W41 tap/untap-multi, W42 drawTargetXp, W43 team-combat-reduction/glyphe-revigorant, W44 each-player-optional/coffre+djakky, W45 costTapResource/smare + 3 free riders hors-starter). Restants, groupés par **sous-système à bâtir** :
...
| **Ressources — production** (W45 : coût FAIT)     | piou-rouge/-jaune/-vert/-bleu (« Produisez une Ressource »), fecaline (condition)                                     | Modèle SANS POOL acté (cf. §2). Coût `costTapResource` **FAIT** (smare). Reste : op **`produceResource`** (no-op de log) pour lever le rappel des 4 Pious (`requiresIncline` déjà en données — l'inclinaison EST la production ; `activateTapPower` exige `atoms.length>0`, d'où un op même vide). |

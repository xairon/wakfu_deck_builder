export function parseHeroFace(filePath: string, $: CheerioAPI): BaseCard {
  try {
    ...
    console.log(
      "Container de sous-types trouvé:",
      subtypeContainer.length > 0 ? "Oui" : "Non",
    );
    ...
    const subtypes = subtypeContainer
      .find("div")
      .map((_, el) => {
    ...
    console.log(
      "Container de stats de base trouvé:",
      baseStatsContainer.length > 0 ? "Oui" : "Non",
    );
    ...
    console.log(
      "Container de niveau/force trouvé:",
      levelContainer.length > 0 ? "Oui" : "Non",
    );
    ...
    levelContainer.find("div").each((_, el) => {
      ...
      if (text.includes("Niveau")) {
        ...
        if (levelMatch) {
          ...
          console.log(
            "Image d'élément trouvée:",
            elementImg.length > 0 ? "Oui" : "Non",
          );
          ...
          if (elementAlt) {
            ...
            console.log(
              `Stat niveau complète: ${JSON.stringify(stats.niveau, null, 2)}`,
            );
          } else {
        ...
        if (forceMatch) {
          ...
          if (elementAlt) {
            ...
            console.log(
              `Stat force complète: ${JSON.stringify(stats.force, null, 2)}`,
            );
          } else {
            console.log("Élément non trouvé dans l'attribut alt");
          }
    ...
    const effectsContainer = $(".row div, div").filter((_, el) =>
      $(el).text().trim().startsWith("Effets :"),
    );
    console.log(
      "Container d'effets trouvé:",
      effectsContainer.length > 0 ? "Oui" : "Non",
    );
    ...
    if (effectsContainer.length) {
      ...
      const effectsList = effectsContainer
        .find("ul li")
        .map((_, el) => {
    ...
    const extensionText = $(".row div, div")
      .filter((_, el) => $(el).text().trim().startsWith("Extension :"))
      .text();
    console.log("Texte de l'extension:", extensionText);

    if (extensionText) {
      const [extension, number] = extensionText
        .replace("Extension :", "")
        .trim()
        .split(/\s+(?=\d)/);
...
function extractBaseStats($: CheerioAPI): BaseStats {
  ...
  if (baseStats.niveau) {
    console.log(
      "Niveau final:",
      baseStats.niveau.value,
      baseStats.niveau.element,
    );
  }
  if (baseStats.force) {
    console.log(
      "Force finale:",
      baseStats.force.value,
      baseStats.force.element,
    );
  }
...
function extractRarity($: CheerioAPI): CardRarity {
  const rarityText = $(".flex-fill.ms-2 .row div:contains('Rareté :')")
    .text()
    .trim();
...
function extractExtension($: CheerioAPI): string {
  const extensionText = $(".flex-fill.ms-2 .row div:contains('Extension :')")
    .text()
    .trim();

    ...
    async function loadCards() {
      try {
        ...
        const loadedCards = result.rows.map((row) => {
          ...
          console.log(
            `Chargement de la carte: ${doc.name} avec l'image: ${doc.imageUrl}`
          )
          return doc
        })
        ...
        setError(
          'Erreur lors du chargement des cartes. Vérifiez la console pour plus de détails.'
        )
        setLoading(false)
      }
    }
  ...
  return (
    <div className="p-4">
      <h1 className="text-2xl font-bold mb-4">
        Ma Collection ({cards.length} cartes)
      </h1>
        ...
        {cards.map((card) => (
          ...
          >
            <img
              ...
              onError={(e) => {
                console.error(
                  `Erreur de chargement de l'image pour ${card.name}:`,
                  card.imageUrl
                )

  ...
  <div class="collection-view w-full px-2">
    <div class="flex flex-col gap-4 max-w-screen-xl mx-auto">
      ...
      <div class="alert alert-info shadow-lg mb-4">
        ...
        <div>
          <h3 class="font-bold">Mode local</h3>
          <div class="text-xs">Votre collection est stockée en local.</div>
        </div>
        <div></div>
      </div>
        ...
        <div v-if="selectedCard" class="space-y-4">
          ...
          <div class="grid md:grid-cols-2 gap-8">
            ...
            <div class="space-y-4">
              ...
              <div
                v-if="selectedCard"
                class="flex flex-col gap-4 bg-base-200 p-4 rounded-box"
              >
                <!-- Cartes normales -->
...
// Gestion des actions
function updateCardQuantity(cardId: string, quantityChange: number, isFoil: boolean) {
  try {
    ...
    const currentQuantity = isFoil ? cardStore.getFoilCardQuantity(cardId) : cardStore.getCardQuantity(cardId)
    ...
    if (quantityChange > 0) {
      ...
      if (newQuantity === 0) {
        const totalRemaining = cardStore.getCardQuantity(cardId) + cardStore.getFoilCardQuantity(cardId)
        if (totalRemaining === 0) {
...
onMounted(async () => {
  try {
    ...
    // Mode local: aucune synchronisation distante
  } catch (error) {
...
function handleImageError() {
  console.error(
    `Erreur de chargement de l'image pour la carte: ${selectedCard.value?.name} (${showVerso.value ? 'verso' : 'recto'})`
  )

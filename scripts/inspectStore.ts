...
const STORE_SNAPSHOT_PATH = path.join(process.cwd(), 'debug', 'store_snapshot.json');
...
async function analyzeStores() {
  try {
    ...
    const filteredStoreFiles = storeFiles.filter(file => !file.includes('__tests__'));
    ...
    console.log(`🔍 Analyse de ${filteredStoreFiles.length} fichiers de store Pinia...`);
    ...
    for (const storeFile of filteredStoreFiles) {
      ...
      const storeName = storeNameMatch ? storeNameMatch[1] : path.basename(storeFile, path.extname(storeFile));
      ...
      const refMatches = storeContent.matchAll(/const\s+(\w+)\s*=\s*(?:ref|reactive|useLocalStorage)<([^>]*)>\(([^)]*)\)/g);
      ...
      const simpleRefMatches = storeContent.matchAll(/const\s+(\w+)\s*=\s*(?:ref|reactive|useLocalStorage)\(([^)]*)\)/g);
      ...
      const computedMatches = storeContent.matchAll(/const\s+(\w+)\s*=\s*computed\(/g);
      ...
      const functionMatches = storeContent.matchAll(/(?:async\s+)?function\s+(\w+)\s*\([^)]*\)/g);
      for (const match of Array.from(functionMatches)) {
        actions.push(match[1]);
      }
      
      // Extraire les fonctions fléchées (arrow functions)
      const arrowFunctionMatches = storeContent.matchAll(/const\s+(\w+)\s*=\s*(?:async\s+)?\([^)]*\)\s*=>/g);

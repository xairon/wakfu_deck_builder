...
async function testCardLoading() {
    ...
    try {
      ...
      if (cards.length > 0) {
        ...
        sample.forEach((card, index) => {
          console.log(`  ${index + 1}. ${card.name || 'Sans nom'} (ID: ${card.id || 'Sans ID'}, Type: ${card.mainType || 'Sans type'})`);
        });
      }
    } catch (error) {
      console.error(`❌ Erreur lors du traitement de ${extension}.json:`, error.message);
    }
  }
  
  console.log(`\n📊 Résumé:`);
  console.log(`Total des cartes: ${totalCards}`);
  console.log(`Extensions: ${Object.keys(extensionStats).length}/${EXTENSIONS.length}`);
  ...
  try {
    ...
    console.error(`❌ Erreur lors de la vérification de collection.json:`, error.message);
  }
}

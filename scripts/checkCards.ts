...
async function checkCardsIntegrity() {
  try {
    ...
    if (cardFiles.length === 0) {
      console.error('❌ Aucun fichier de données de cartes trouvé dans', DATA_DIR);
      return;
    }
    ...
    console.log(`📊 ${filteredCardFiles.length} fichiers de données de cartes trouvés.`);
    ...
    console.log(`📋 Champs: ${report.cardsWithMissingFields.length} cartes avec champs manquants`);

    ...
    try {
        ...
        switch (mainType) {
            case 'Équipement':
                ...
                $('div').each((_, el) => {
                    const text = $(el).text().trim();
                    if (text.includes('Niveau requis') || text.includes('Métier requis')) {
...
async function analyzeAllTypes(): Promise<void> {
    ...
    const outputPath = path.join(__dirname, '..', 'analysis', 'types_analysis.json');
    ...
    for (const [type, data] of Object.entries(analysis)) {
        ...
        console.log(`  Caractéristiques uniques: ${Array.from(data.characteristics).length}`);
    }
    ...
    for (const [type, data] of Object.entries(analysis)) {
        if (data.examples.length > 0) {
            const examplePath = path.join(examplesDir, `${type.toLowerCase().replace(/\s+/g, '_')}_example.html`);

...
function parseStats($: cheerio.CheerioAPI, statsDiv: cheerio.Cheerio<cheerio.Element>): BaseStats {
...
function parseEffects($: cheerio.CheerioAPI, effectsDiv: cheerio.Cheerio<cheerio.Element>): CardEffect[] {
  ...
  effectsDiv.find('li').each((_, el) => {
    ...
    if (tokenMatch) {
      ...
      effect.linkedTokens = [{
        ...
        elements: tokenElements.length > 0 ? tokenElements.map(c => c.element) : undefined
      }];
    }
...
function parseKeywords($: cheerio.CheerioAPI, keywordsDiv: cheerio.Cheerio<cheerio.Element>): CardKeyword[] {
  ...
  keywordsDiv.find('li').each((_, el) => {
    ...
    if (name) {
      ...
      const htmlContent = $.html(el);
      if (htmlContent.includes('symbole-ressource')) {
        keyword.elements = parseElementalCosts(htmlContent).map(c => c.element);
      }
...
function parseBaseStats($: cheerio.CheerioAPI, statsDiv: cheerio.Cheerio<cheerio.Element>): BaseStats {
  const stats: BaseStats = {};
...
function parseExtensionInfo($: cheerio.CheerioAPI): ExtensionInfo {
  const extensionDiv = $('div').filter((_, el) => {
    return $(el).text().trim().startsWith('Extension :');
  }).last();

  if (extensionDiv.length === 0) return { name: '', number: '', shortUrl: undefined };
...
function parseElementalStat($: cheerio.CheerioAPI, text: string): ElementalStat | undefined {
...
function parseArtists($: cheerio.CheerioAPI): string[] {
  const artistsDiv = $('div').filter((_, el) => {
  ...
  return artistsText
    ...
    .filter(artist => artist.length > 0 && !artist.includes('Niveau :') && !artist.includes('Élément :'));
}

// Fonction pour parser les éléments
function parseElements($: cheerio.CheerioAPI, container: cheerio.Cheerio<cheerio.Element>): CardElement[] {
...
function parseRarity($: cheerio.CheerioAPI): CardRarity {
  const rarityDiv = $('div').filter((_, el) => {

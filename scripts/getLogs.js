...
function getProcessLogs() {
  try {
    ...
    if (os.platform() === 'win32') {
      cmd = 'powershell -Command "Get-Process node | Where-Object {$_.CommandLine -match \'ts-node\'} | Select-Object Id"';
    } else {
...
function getRecentConsoleErrors() {
  try {
    ...
    const errorLines = logs.split('\n')
      .filter(line => line.includes('❌ Erreur') || line.includes('Error:') || line.includes('ERROR'))
...
// Fonction principale
function main() {
  console.log("📝 Récupération des logs et informations de l'application Wakfu Deck Builder...\n");

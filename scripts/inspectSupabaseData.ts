...
// Configuration
const REPORT_PATH = path.join(process.cwd(), 'debug', 'supabase_data_report.json');
...
async function inspectSupabaseData() {
  try {
    ...
    if (!session?.user) {
      console.log('❌ Utilisateur non authentifié. Impossible d\'inspecter les données.');
      return;
    }
    ...
    console.log(`Nom d'utilisateur: ${report.profile?.username || 'Non défini'}`);

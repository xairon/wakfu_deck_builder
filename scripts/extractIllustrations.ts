 ...
 *
 * Source:  public/images/cards/<id>.png        (and <id>_recto.png for heroes)
...
const BAND = { left: 0.03, top: 0.14, width: 0.94, height: 0.44 };
const HERO_BAND = { left: 0.03, top: 0.1, width: 0.72, height: 0.52 };
...
function collectPngFiles(): string[] {
  return (
    fs
      ...
      .filter((n) => !n.endsWith("_verso.png"))
  );
}
...
async function extractOne(
  ...
  const id = fileName.replace(/\.png$/i, "").replace(/_recto$/i, "");
  ...
  try {
    ...
    const band = /_recto\.png$/i.test(fileName) ? HERO_BAND : BAND;

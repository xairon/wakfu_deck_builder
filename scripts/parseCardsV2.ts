...
function extractElementalStat($: CheerioAPI, selectors: { container: string; value: string; element: string }): ElementalStat {
...
function parseEffects($: CheerioAPI): CardEffect[] {
  return $(SELECTORS.effects.items).map((_, el) => {
    const $el = $(el);
    ...
    return {
      ...
      elements: $el.find('img.symbole-ressource').map((_, img) => 
        parseElementFromImg($, img)
      ).get()
    };
  }).get();
}
...
function parseKeywords($: CheerioAPI): CardKeyword[] {
  return $(SELECTORS.keywords.items).map((_, el) => {
    const $el = $(el);
    const [name, description = ''] = cleanString($el.text()).split(':').map(s => s.trim());
...
function parseArtists($: cheerio.CheerioAPI): string[] {
  ...
  const artists = artistsText.replace('Artistes : ', '').split(',').map(a => a.trim());
...
function parseExtensionInfo($: cheerio.CheerioAPI): ExtensionInfo {
  ...
  const [_, name, number] = extensionText.match(/Extension\s*:\s*([^0-9]+)(?:\s*(\d+\/\d+))?/) || [];
...
function parseElements($: cheerio.CheerioAPI, container: cheerio.Cheerio<any>): CardElement[] {
...
function parseRarity($: cheerio.CheerioAPI): CardRarity {
  const rarityText = $('div:contains("Rareté :")').find('.badge').attr('title')?.toLowerCase() || '';
  
  switch (rarityText) {
    case 'peu commune': return 'Peu Commune';
    case 'rare': return 'Rare';
    case 'mythique': return 'Mythique';
    case 'légendaire': return 'Légendaire';
...
function parseFlavor($: cheerio.CheerioAPI): { text: string; attribution?: string } | undefined {
...
function parseHero(filePath: string): HeroCard {
  ...
  const basePath = isFirstFace ? filePath.replace(/\.html$/, '') : filePath.replace('-2.html', '');
  ...
  // Extraction du nom
  const cardName = cleanString($recto(SELECTORS.cardName).contents().filter(function() {
  ...
  const rarity = cleanString($recto(SELECTORS.rarity).attr('title') || '') as CardRarity;
...
async function parseCard(filePath: string): Promise<Card | null> {
  try {
    ...
    const cardName = cleanString($(SELECTORS.cardName).contents().first().text());
    ...
    const elements = $(SELECTORS.stats.container).find('img.symbole-ressource')
    ...
    const baseCard = {
      ...
      artists: $(SELECTORS.artists).text()
        .replace('Artistes :', '')
        .split(',')
...
async function parseAllCards() {
  ...
  const htmlFiles = files.filter((file: string) => file.toString().endsWith('.html'));
  ...
  if (parsingErrors.length > 0) {
    ...
    console.log(`${parsingErrors.length} erreurs de parsing sauvegardées dans ${errorPath}`);
  }

  console.log('\n=== Résumé du parsing ===');
  console.log(`Total des cartes : ${totalCards}`);
  console.log(`Cartes parsées avec succès : ${successfullyParsed}`);
  console.log(`Taux de réussite : ${((successfullyParsed / totalCards) * 100).toFixed(2)}%`);
  console.log('\n=== Fin du parsing ===');
}

  ...
  try {
    ...
    if (session?.user) {
      ...
      if (!profileError && profileData) {
        ...
        report.profileError = profileError ? profileError.message : 'Profil non trouvé';
      }
    }
    ...
    if (report.isAuthenticated) {
      ...
      console.log(`Email confirmé: ${report.user.emailConfirmed ? '✅ Oui' : '❌ Non'}`);
      console.log(`Session expire: ${new Date(session.expires_at! * 1000).toLocaleString()}`);

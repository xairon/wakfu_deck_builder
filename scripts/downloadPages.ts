...
async function downloadPage(
  ...
  try {
    ...
    const extensionName = card.extension
      .toLowerCase()
      .normalize('NFD')
...
async function processExtension(
  ...
  const downloadPromises = cards.map((card, index) => {
    return limit(async () => {
      console.log(
        `[${index + 1}/${cards.length}] Traitement de ${card.name}...`
      )
...
async function downloadAllPages() {
  console.log(
    `Configuration: ${NUM_CPUS} cœurs détectés, ${MAX_CONCURRENT_REQUESTS} requêtes simultanées maximum`
  )
  ...
  const cardsByExtension = cards.reduce(
    (acc, card) => {

...
async function testSupabaseSync() {
  try {
    ...
    if (!session?.user) {
      console.log('❌ Utilisateur non authentifié. Impossible de tester la synchronisation.');
      return;
    }
    ...
    try {
      ...
      report.storeStatus.collectionCardsCount = Object.keys(report.localData.collection).length;
    ...
    try {
      ...
      console.error('❌ Erreur lors de la récupération des données distantes:', error);
      report.syncResults.errors.push({
    ...
    if (supabaseStore.isOnline) {
      try {
        ...
        if ('synchronizePendingChanges' in supabaseStore) {
          ...
          report.syncResults.collectionItemsSynced = Object.keys(newCollectionState).length;
          
          report.syncResults.decksSynced = supabaseStore.decks.length;
          
          console.log(`✅ Synchronisation réussie en ${report.syncResults.timeElapsed}ms`);
        } else {
      ...
      console.log('⚠️ Mode hors ligne actif. La synchronisation ne sera pas exécutée.');
      report.syncResults.errors.push({
        phase: 'synchronization',
    ...
    console.log(`Mode en ligne: ${report.storeStatus.isOnline ? '✅ Oui' : '❌ Non'}`);
    console.log(`Éléments dans la collection locale: ${report.storeStatus.collectionCardsCount}`);
    console.log(`Decks locaux: ${report.storeStatus.decksCount}`);
    console.log(`Éléments dans la collection distante: ${report.remoteData.collection.length}`);
    ...
    if (report.syncResults.success) {
      ...
      console.log(`Cartes synchronisées: ${report.syncResults.collectionItemsSynced}`);
